...
void GL::Font::Build(int height) {
	...
	wglUseFontBitmaps(hdc, 32, 96, base); // ? define range of characters what we need
...
void GL::Font::Print(float x, float y, const unsigned char color[3], const char* format, ...) {
	...
	glRasterPos2f(x, y); // -> where you start drawing
	char text[100];
...
vector3d GL::Font::centerText(float x, float y, float width, float height, float textWidth, float textHeight) {
	vector3d text;
